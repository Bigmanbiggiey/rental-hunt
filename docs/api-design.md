...
**`enforce_viewing_request_status_transition()`'s rejection (Sprint 10 security hardening, added 2026-08-07, `decisions.md` ADR-038):** a new `BEFORE UPDATE` trigger on `viewing_requests` (`database.md` §9) codifying the same forward-only status transition set `viewingRequestRepository` already enforced client-side, now backstopped at the DB level. Raises a dedicated errcode, `RH004`, mapped to the existing `INVALID_STATE_TRANSITION` `ErrorCode` — reusing `RH002`'s mapping rather than adding a distinct one, since it's semantically identical (an invalid source→destination transition). In normal use through the Repository this trigger should never actually fire (the Repository's own `.eq('status', ...)` guards already prevent the same invalid transitions) — it exists purely as the RLS-bypass backstop, the same role `prevent_booking_unavailable_property()` plays for booking availability.
