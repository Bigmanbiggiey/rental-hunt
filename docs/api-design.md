...
| **JWT** | Tokens are never persisted in `localStorage`; `supabase-js`'s default secure storage handling is used as-is. Tokens are never logged, including in error reports. |
