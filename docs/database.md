...
**Security hardening (Sprint 10, `20260807090000_security_hardening.sql`, `decisions.md` ADR-038) — three more triggers in the same "RLS checks row ownership, a trigger locks the columns that must stay fixed" family as `prevent_self_role_change()`/`enforce_verification_authority()` above:**

- **`prevent_agent_self_update_overreach()`** (`BEFORE UPDATE` on `agents`) — `agents_update_own_agent`'s `USING`/`WITH CHECK` only ever verified `profile_id = auth.uid()`, never constraining which columns changed. Found via a whole-project security review: an agent could `PATCH` their own `agents` row and set `agency_id` to any other agency's id, inheriting that agency's write access on `properties`/`property_images`/`property_amenities`/the `property-images` Storage bucket (all gated on `current_agency_id()`, which reads live from `agents.agency_id`) and even self-approve their own still-`pending_review` application via `agencies_update_own_agent`. The trigger now rejects any self-update that touches `id`/`profile_id`/`agency_id`/`is_active`/`deleted_at`, narrowing self-service UPDATE to exactly `bio`/`job_title` — the scope the Policy Summary row above already documented as intended, which RLS alone had never actually enforced.
- **`prevent_viewing_request_identity_change()`** (`BEFORE UPDATE` on `viewing_requests`) — `viewing_requests_update_own_agent` similarly only checked `agent_id = current_agent_id()`, never constraining `customer_id`/`property_id`/`agent_id` themselves. An agent could rewrite `customer_id` on a request they control to an arbitrary profile id, then read that profile via `profiles_select_own_customers_by_agent` (`agent_dashboard.sql`) — reaching any profile in the system, including admin/moderator accounts, since `profiles` is one shared table with no column-level grant narrowing. The trigger now makes `customer_id`/`property_id`/`agent_id` immutable after INSERT for any non-`service_role` caller; no legitimate flow (customer or agent) ever needed to change them.
- **`enforce_viewing_request_status_transition()`** (`BEFORE UPDATE` on `viewing_requests`) — RLS placed no restriction at all on the agent-side `status` transition (only the customer-side `viewing_requests_cancel_own_customer` pinned source/destination status), so an agent could flip any of their own requests straight to `completed` with no `confirmed` step, contrary to this project's own "RLS is the sole authority, a Service-layer check is never the real security boundary" model (`CLAUDE.md` §12) — the Repository-layer state machine (`viewing-request.repository.ts`) had no DB-level backstop. The trigger now codifies exactly the same transition set the Repository already enforces (`pending→confirmed`, `pending/confirmed→cancelled`, `confirmed→completed`, `confirmed→no_show`; any update that leaves `status` unchanged, e.g. reschedule, is always allowed) and raises a dedicated errcode, `RH004` (mapped to the existing `INVALID_STATE_TRANSITION` `ErrorCode`, reusing `RH002`'s precedent rather than adding a new one). **Deliberately does not** also gate on `requested_date`/`requested_time` having passed — see the migration's own header note and ADR-038's Consequences section for the acknowledged residual risk this leaves open (a Low-severity, self-serving review-rating fraud path, bounded to an agent's own data).
...
| `agents` | SELECT (public directory fields via a view) | SELECT | SELECT all; UPDATE own row limited to `bio`/`job_title` — trigger-enforced since Sprint 10 (see column-level note below), `agency_id` reassignment is not possible | SELECT all | Full CRUD |
...
| `viewing_requests` | No access | SELECT/INSERT own rows; UPDATE own row only to set `status = 'cancelled'` while `status IN ('pending','confirmed')` | SELECT/UPDATE rows where `agent_id` is their own (confirm, reschedule, cancel, complete, no-show) — `customer_id`/`property_id`/`agent_id` are trigger-immutable and `status` follows a trigger-enforced state machine, both since Sprint 10 (see below) | SELECT all | Full CRUD |
...
**Column-level note on `agents`:** same limitation, same fix — `prevent_agent_self_update_overreach()` (Sprint 10, see above) restricts an agent's own-row UPDATE to `bio`/`job_title`, since RLS's row-level `agents_update_own_agent` policy can't itself exclude the `agency_id` column.
