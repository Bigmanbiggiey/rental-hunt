...
Both are `security definer` so they can read `profiles`/`agents` regardless of the calling user's own RLS visibility, and `stable` so the planner can cache them within a statement.
...
| `properties` | SELECT where `is_archived = false AND deleted_at IS NULL AND verification_status <> 'rejected'` | Same as Guest | SELECT all rows for own `agency_id`; INSERT/UPDATE (except verification columns, see above) scoped to own agency | SELECT all; UPDATE limited to verification via RPC | Full CRUD |
