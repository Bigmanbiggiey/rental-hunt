...
| **Monitoring integration** | **Built Sprint 10 (2026-08-06), exactly as this row originally planned.** `shared/lib/sentry.ts` wraps `@sentry/react`'s minimal error-capture init (no BrowserTracing/Replay — bundle-size checked, ~+10.5KB gzip on the shared eager chunk, an accepted cost since Sentry must load before any error can occur, unlike Leaflet/recharts' page-scoped lazy-loading), called once at `main.tsx` startup. `logger.error()` now also calls `Sentry.captureMessage()` — no other call site anywhere in the app changed, exactly the "swap the transport" plan this row described. Reads `VITE_SENTRY_DSN`; unset (every local/dev environment) means the SDK is disabled entirely, not erroring. |
