...
## ADR-033: Session-management hardening — server-side password policy, sign-out-of-other-devices, admin/moderator idle timeout, dev-only per-tab sessions

**Status:** Accepted
**Date:** 2026-08-04

**Decision:** Following a developer request to review this project's session-management posture given Supabase Auth as the backend, four independent changes were made:

1. **Server-side password policy now matches the already-documented client-side one.** `supabase/config.toml`'s `[auth.email]` `minimum_password_length`/`password_requirements` were `6`/`""` (Supabase's own weak defaults) despite `passwordSchema` (`entities/user/user.schema.ts`) and `api-design.md` §14 already requiring 8 characters + a letter + a digit since Sprint 2 — meaning a direct API call bypassing the frontend could set a 6-character, no-complexity password. Raised to `8`/`"letters_digits"` (not the stricter `lower_upper_...` variants, which would reject passwords the frontend itself already accepts, since `passwordSchema`'s `[A-Za-z]` check doesn't require both cases).
2. **"Sign out of all other devices"** (`features/profile-management`, `ProfilePage`) — `credentialsRepository.signOutOtherDevices()` calls `supabase.auth.signOut({ scope: 'others' })`, deliberately not `'global'`: it revokes every *other* session's refresh token while leaving the one the user is actively using (and clicked the button from) signed in, matching Google/GitHub/Slack's own "sign out everywhere else" shape rather than also signing the caller out of the page they're on.
3. **A 15-minute idle timeout, admin/moderator only** (`features/authentication`'s `useIdleTimer` + `IdleSessionGuard`, mounted once in `App.tsx`). A 60-second warning dialog ("Stay signed in") precedes the actual sign-out; any tracked activity (mouse/keyboard/scroll/touch) resets the clock, including while the warning is showing. Scoped to `admin`/`moderator` only — those roles can approve listings and manage other users' accounts, a materially higher-value target for an unattended, still-authenticated tab than a `customer`/`agent` session; `enabled: false` means no timers or listeners run at all for other roles, not just a longer timeout.
4. **`sessionStorage` instead of `localStorage` for the Supabase client, `npm run dev` only** (`shared/lib/supabase.ts`, gated on a new `env.isDev`). Since `sessionStorage` is scoped per-tab (unlike `localStorage`, shared across every tab on the same origin), this gives each browser tab its own independent session automatically — a developer can open the real app in three tabs and be signed in as `admin`, `agent`, and `customer` simultaneously, with no custom testing tool. Never applied in production: a real user opening a link in a new tab expects their existing session to carry over, which `sessionStorage` would break.

**Context:** The developer's own manual testing had run into a genuine limitation — unable to have admin, agent, and customer all signed in in the same browser at once to exercise cross-role flows (e.g., the Verification Review Page's approve/reject from one role while checking the result from another). Asked for best-practice session-management recommendations given Supabase Auth specifically, which surfaced #1 as a real, previously-undiscovered gap and #2/#3 as genuine hardening opportunities neither `api-design.md` nor `database.md` had previously addressed (session lifecycle was never a documented topic before this).

**Rationale:** All four are scoped to their actual risk/benefit, not applied uniformly out of caution. The password-policy fix closes a real gap between documented intent and enforced reality — the kind of drift `CLAUDE.md` §8 exists to prevent, caught here because the frontend's own rule was already the correct spec, just never mirrored server-side. `scope: 'others'` over `'global'` for the sign-out feature is a deliberate, standard UX choice — a security feature that also logs you out of the device you're using to invoke it is more likely to be avoided than used. The idle timeout's role-scoping avoids a blanket UX cost (a customer casually browsing shouldn't get logged out reading a long listing) for a security benefit concentrated on the roles that actually carry it. The dev-only `sessionStorage` swap was chosen over building a bespoke multi-actor testing tool (a real alternative considered and rejected) — it reuses a browser-native isolation mechanism instead of new application code, and disappears entirely from the production bundle path (`env.isDev` is statically `false` in a production build, so the conditional branch is dead code, not a runtime check).

Two real UX decisions for the idle timeout (duration; warn-then-sign-out vs. silent) were put to the developer via `AskUserQuestion` rather than assumed — both timeout duration and warning-vs-silent behavior are product calls with no "obviously correct" answer the docs could resolve.

**Consequences:** `docs/api-design.md` §5 gains a new Sign Out of Other Devices contract entry. `ui-guidelines.md` gains new patterns for the Sessions section (Profile page) and the idle-timeout warning dialog. Local `supabase/config.toml` and the production Supabase Auth dashboard now have one more setting pair to keep in sync — the same local/remote Auth-config parity gap already tracked in `project-state.md`'s Technical Debt (open since Sprint 2), not a new category of drift, just one more concrete item on that existing checklist. `useIdleTimer` is generic (no auth-specific knowledge) and reusable if a future role ever needs a different idle policy. A React Compiler lint rule (calling `setState` synchronously inside a `useEffect` body) was hit and fixed while building `useIdleTimer` — worth remembering as a real constraint on this codebase's effect-based hooks going forward, not just this one.

**Related Documents:** `api-design.md` §5/§14, `ui-guidelines.md` (new Sessions/idle-timeout patterns), `coding-standards.md` §21 (`service_role`/secret-handling precedent this doesn't touch, but the "server-side enforcement is the real boundary" principle it shares), `project-state.md` Technical Debt (local/remote Auth-config parity, unchanged in kind by this ADR).
...
## ADR-038: Security-review findings closed via trigger backstops on `agents`/`viewing_requests`, superseding Sprint 6's "status-machine legality is a Repository/Service concern, not RLS" call for the agent-side `viewing_requests` transition

**Status:** Accepted
**Date:** 2026-08-07

**Decision:** Following a whole-project security review (self-triggered `/security-review`, not tied to any single PR diff), three trigger backstops were added in `20260807090000_security_hardening.sql`: (1) `agents.agency_id`/`profile_id`/`is_active`/`deleted_at` are now immutable on an agent's own-row UPDATE, narrowing it to `bio`/`job_title`; (2) `viewing_requests.customer_id`/`property_id`/`agent_id` are now immutable on UPDATE; (3) `viewing_requests.status` now follows an explicit forward-only state machine enforced by a trigger, matching exactly the transition set `viewing-request.repository.ts` already enforced client-side. All three ship as `service_role`-bypassable triggers, the same pattern already established by `prevent_self_role_change()`/`enforce_verification_authority()`.

**Context:** The review found two HIGH-confidence, independently-verified (9/10 confidence each, via a dedicated false-positive-filtering pass per file/line/exploit-chain) vulnerabilities reachable via raw PostgREST calls, bypassing the frontend entirely:

1. `agents_update_own_agent` (`property_discovery.sql`) checked only row ownership (`profile_id = auth.uid()`), never constraining `agency_id` — any agent-role account (obtainable via the normal self-service agency application + admin-approval flow, no insider access needed) could reassign themselves into any other agency, since `current_agency_id()` reads live from `agents.agency_id` and gates nearly every other agent-write policy (`properties`, `property_images`, `property_amenities`, the `property-images` Storage bucket, and `agencies_update_own_agent` itself — including a path to self-approve a still-`pending_review` application).
2. `viewing_requests_update_own_agent` (`customer_experience.sql`) checked only `agent_id = current_agent_id()`, never constraining `customer_id`/`property_id`. Combined with `viewing_requests_insert_own_customer` placing no check on which `agent_id` a customer names, an agent could fabricate a viewing request naming themselves, rewrite its `customer_id` to an arbitrary profile id, and read that profile via `profiles_select_own_customers_by_agent` (`agent_dashboard.sql`) — reaching any profile in the system, including admin/moderator accounts, since `profiles` is one shared table.

A third, related finding — an agent flipping a fabricated viewing request straight to `completed` (no `confirmed` step) to submit a self-serving fake review — was independently verified as concretely exploitable (8/10) but rated **Low**, not Medium/High: the blast radius is bounded to the agent's own data (a sock-puppet customer account and their own agency's properties), not a cross-tenant confidentiality/authorization breach.

That third finding directly touches a documented prior decision: `viewing-request.repository.ts`'s own code comments record a "Sprint 6 plan's Part B decision" to keep viewing-request status-machine legality a Repository/Service-layer concern, not an RLS one, accepting "RLS is ownership-only for the agent path" as a deliberate trade-off at the time. `CLAUDE.md` §12's overarching security model — "RLS is the sole authority... Service-layer checks exist for fast, specific errors, never as the actual security boundary" — was not applied to that specific decision when it was made. Surfaced here rather than silently overridden, per `CLAUDE.md` §14: the developer's explicit "make the changes" instruction, given directly after reviewing and accepting this security report's findings, is treated as the deliberate override, and this ADR is the required decision-record entry documenting what it supersedes and why (`CLAUDE.md` §8's "Breaking change" row).

**Rationale:** Every fix follows this project's own pre-existing, established pattern (`prevent_self_role_change()`, `enforce_verification_authority()`) for the exact class of gap involved — an RLS `UPDATE` policy that correctly scopes *which row* can be touched but can't (RLS is row-level, not column-level) restrict *which columns* on that row may change. A trigger is the documented, precedented fix; no new pattern was introduced. The status-transition trigger closes a real DB-level gap in the "RLS is the sole authority" model without changing any legitimate, tested application behavior — it codifies exactly the transition set the Repository already enforced and already tested, so no application-facing behavior change.

**Consequences:** No known legitimate flow is affected — grepped the full frontend and confirmed no code path ever updates `agents.agency_id`/`profile_id`/`is_active`/`deleted_at`, or `viewing_requests.customer_id`/`property_id`/`agent_id`, or performs a status transition outside the now-encoded set; the full test suite (405 tests, including the new regression tests below) passes unchanged in isolation (the usual pre-existing full-suite resource-contention flakes aside, unrelated to this change and individually confirmed passing). New RLS regression tests (`agent.rls.test.ts`, `viewing-request.rls.test.ts`) and a schema-level test (`applyForAgency.schema.test.ts`, for the separately-fixed XSS finding below) prove each closed path stays closed. **Acknowledged residual risk, deliberately not closed here:** the Low-severity review-rating fraud path (an agent completing a fabricated viewing request they legitimately confirmed first) is not fully eliminated — closing it fully would require either identity/interaction verification beyond a trigger's reach, or a `requested_date`/`requested_time`-based completion gate, which was considered and rejected for this change (see the migration's own header note) because it would change real booking-flow behavior (disallowing a legitimate early completion) for a Low-severity, self-contained-fraud finding. Left open as documented technical debt rather than addressed with a broader, riskier change.

**Related Documents:** `database.md` §9 (Policy Summary rows for `agents`/`viewing_requests` corrected, new trigger writeups added, in the same change as this ADR), `api-design.md` §15.3 (new `RH004` errcode, mapped to the existing `INVALID_STATE_TRANSITION` `ErrorCode`, reusing `RH002`'s precedent), `supabase/migrations/20260807090000_security_hardening.sql`.

**Separately, in the same session (not requiring an ADR — a straightforward input-validation bug fix, not an architectural decision):** a stored-XSS finding (agency `social_links`/`logoUrl` accepting `javascript:`/`data:` URIs, since Zod's `.url()` checks syntax, not scheme) was fixed by adding a shared `isHttpUrl()` helper (`shared/lib/url.ts`) used both at the Zod-validation boundary (`applyForAgency.schema.ts`, `admin-agencies/createAgency.schema.ts`) and defensively at the `AgencyDetailPage.tsx` render sink.
